package com.mcwonders.uikit.session.helper;
...
import com.mcwonders.uikit.common.util.C;
import com.mcwonders.uikit.common.util.file.AttachmentStore;
import com.mcwonders.uikit.common.util.file.FileUtil;
import com.mcwonders.uikit.common.util.storage.StorageType;
import com.mcwonders.uikit.common.util.storage.StorageUtil;
import com.mcwonders.uikit.common.util.string.MD5;
import com.mcwonders.uikit.session.activity.CaptureVideoActivity;
...
public class VideoMessageHelper {
    ...
    public void showVideoSource(int local, int capture) {
        ...
        dialog.setTitle(activity.getString(com.mcwonders.uikit.R.string.input_panel_video));
    ...
    protected void chooseVideoFromLocalKitKat() {
        ...
        try {
            ...
            Toast.makeText(activity, com.mcwonders.uikit.R.string.gallery_invalid, Toast.LENGTH_SHORT).show();
    ...
    public void onGetLocalVideoResult(final Intent data) {
        ...
        if (AttachmentStore.copy(filePath, md5Path) != -1) {
            ...
            Toast.makeText(activity, com.mcwonders.uikit.R.string.video_exception, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private boolean checkVideoFile(String file) {
        ...
        if (new File(file).length() > C.MAX_LOCAL_VIDEO_FILE_SIZE) {
            Toast.makeText(activity, com.mcwonders.uikit.R.string.im_choose_video_file_size_too_large, Toast.LENGTH_SHORT).show();
            return false;
        }

        if (!StorageUtil.isInvalidVideoFile(file)) {
            Toast.makeText(activity, com.mcwonders.uikit.R.string.im_choose_video, Toast.LENGTH_SHORT).show();
            return false;
        }
        return true;
    }
