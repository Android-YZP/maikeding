package com.mcwonders.uikit.session.emoji;
...
public class EmojiAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		convertView = LayoutInflater.from(context).inflate(com.mcwonders.uikit.R.layout.nim_emoji_item, null);
		ImageView emojiThumb = (ImageView) convertView.findViewById(com.mcwonders.uikit.R.id.imgEmoji);
		int count = EmojiManager.getDisplayCount();
		int index = startIndex + position;
		if (position == EmoticonView.EMOJI_PER_PAGE || index == count) {
			emojiThumb.setBackgroundResource(com.mcwonders.uikit.R.drawable.nim_emoji_del);
		} else if (index < count) {
