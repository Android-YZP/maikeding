package com.mcwonders.mkd.location.activity;
...
import com.mcwonders.mkd.location.adapter.IconListAdapter;
import com.mcwonders.mkd.location.helper.MapHelper;
import com.mcwonders.mkd.location.helper.NimLocationManager;
import com.mcwonders.uikit.common.ui.dialog.CustomAlertDialog;
import com.mcwonders.uikit.common.util.string.StringUtil;
...
public class NavigationAmapActivity extends UI implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(com.mcwonders.mkd.R.layout.map_view_amap_navigation_layout);
		mapView = (MapView) findViewById(com.mcwonders.mkd.R.id.autonavi_mapView);
		mapView.onCreate(savedInstanceState);// 此方法必须重写

		ToolBarOptions options = new ToolBarOptions();
		setToolBar(com.mcwonders.mkd.R.id.toolbar, options, com.mcwonders.mkd.R.id.toolbar_amap_navigation_title);
	...
	private void initView() {
		sendButton = findView(com.mcwonders.mkd.R.id.action_bar_right_clickable_textview);
		sendButton.setText(com.mcwonders.mkd.R.string.location_navigate);
		sendButton.setOnClickListener(this);
		sendButton.setVisibility(View.INVISIBLE);

		myLocationFormatText = getString(com.mcwonders.mkd.R.string.format_mylocation);
	}
	...
	private void initLocation() {
		...
		if (TextUtils.isEmpty(desAddressInfo)) {
			desAddressInfo = getString(com.mcwonders.mkd.R.string.location_address_unkown);
		}
	...
	private void updateSendStatus() {
		...
		if (TextUtils.isEmpty(myAddressInfo)) {
			setTitle(com.mcwonders.mkd.R.string.location_loading);
			sendButton.setVisibility(View.GONE);
		} else {
			setTitle(com.mcwonders.mkd.R.string.location_map);
			sendButton.setVisibility(View.GONE);
		}
	}
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case com.mcwonders.mkd.R.id.action_bar_right_clickable_textview:
			navigate();
			break;
		}
	}
	...
	public void onLocationChanged(NimLocation location) {
		if (location != null && location.hasCoordinates()) {
			if (firstLocation) {
				...
				// 缩放到可见区
				int boundPadding = getResources().getDimensionPixelSize(com.mcwonders.mkd.R.dimen.friend_map_bound_padding);
	...
	private void showLocationFailTip() {
		if (firstLocation && firstTipLocation) {
			...
			myAddressInfo = getString(com.mcwonders.mkd.R.string.location_address_unkown);
			Toast.makeText(this, com.mcwonders.mkd.R.string.location_address_fail, Toast.LENGTH_LONG).show();
		}
	}
	...
	private MarkerOptions defaultMarkerOptions() {
		...
		markerOptions.icon(BitmapDescriptorFactory.fromResource(com.mcwonders.mkd.R.drawable.pin));
	...
	private void doNavigate(final NimLocation origin, final NimLocation des) {
		...
		if (infos.size() >= 1) {
			...
			dialog.setTitle(getString(com.mcwonders.mkd.R.string.tools_selected));
			dialog.show();
		} else {
			IconListAdapter.IconListItem item = new IconListAdapter.IconListItem(getString(com.mcwonders.mkd.R.string.friends_map_navigation_web), null, null);
			items.add(item);
			CustomAlertDialog dialog = new CustomAlertDialog(this, items.size());
			dialog.setAdapter(adapter, new DialogInterface.OnClickListener() {
				@Override
	...
	private View getMarkerInfoView(Marker pmarker) {
		...
		View view = getLayoutInflater().inflate(com.mcwonders.mkd.R.layout.amap_marker_window_info, null);
		TextView textView = (TextView) view.findViewById(com.mcwonders.mkd.R.id.title);
